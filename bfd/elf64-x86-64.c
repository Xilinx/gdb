			  ...
			  const Elf_Internal_Rela *relocs)
{
  ...
  for (rel = relocs; rel < relend; rel++)
	...
	{
	case R_X86_64_TLSLD:
	  if (elf_x86_64_hash_table (info)->tls_ld_got.refcount > 0)
	    elf_x86_64_hash_table (info)->tls_ld_got.refcount -= 1;
	  break;
				 ...
				 struct bfd_link_info *info)
{
  ...
  if (tls_sec)
      ...
      if (tlsbase && tlsbase->type == STT_TLS)
	{
	  ...
	  elf_x86_64_hash_table (info)->tls_module_base = bh;
...
elf_x86_64_set_tls_module_base (struct bfd_link_info *info)
{
  ...
  base = elf_x86_64_hash_table (info)->tls_module_base;
